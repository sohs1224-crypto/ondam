...
function signupScreen(){
  ...
  var dis = f.busy || authInvalidSignup() ? ' disabled' : '';
  return '<div class="auth">'+
    ...
    '<div style="margin-top:20px"><button class="btn btn--primary" id="authSubmit" data-action="authSignup"'+dis+'>'+(f.busy?'잠시만요…':'회원가입')+'</button></div>'+
    errBox(f.authError)+
...
/* render 후 로그인·아이디찾기·비번찾기 버튼을 항상 활성화 (busy 제외) */
(function(){
  ...
  window.render = function(){
    ...
    ['loginBtn','findIdBtn','resetPwBtn'].forEach(function(bid){
